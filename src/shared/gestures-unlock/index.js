 ...
 */
import {checkSupportTouch} from "../functions/common";

const GesturesUnlock = function ({canvas}) {
  this.canvas = canvas;
  this.radius = 20; // 圆的半径
...
GesturesUnlock.prototype.reNewLine = function (ctx) {
  var thisObj = this;
  var canvas = thisObj.canvas;
  // 获取画布宽高
  var _guaWidth = canvas.width;
  var _guaHeight = canvas.height;
  // 获取画布距离页面顶端和左端的距离
  var _leftSpace = canvas.offsetLeft;
  var _topSpace = canvas.getBoundingClientRect().top + window.scrollY;


  // 9个点的全部坐标
  var pointArr = thisObj.ninePoint();
  const onTouchMove = function (eve) {
    if (!this.started) return;
    // 取消页面的触摸移动默认动作
    eve.preventDefault();
    // 触点位置坐标减去偏移量
    const {clientX, clientY} = thisObj.supportTouch ? eve.targetTouches[0] : eve
    var touchX = clientX - _leftSpace;
    var touchY = clientY - _topSpace;
  ...
  canvas.addEventListener(thisObj.supportTouch ? 'touchmove' : "mousemove", onTouchMove);
}
export default GesturesUnlock
