...
const pickerTpl = `<div class="<%= className %>">
...
`
const groupTpl = `<div class="weui-picker__group">
...
export function picker() {
  ...
  function scroll(items, level) {
    ...
    if (window.__lineTemp) {
      lineTemp = window.__lineTemp;
    }
    if (lineTemp[level] === undefined && defaultValue && defaultValue[level] !== undefined) {
      // 没有缓存选项，而且存在defaultValue
      const defaultVal = defaultValue[level];
  ...
  const picker_scroll = (lineTemp) => {
    window.__lineTemp = lineTemp
    scroll(items, 0)
  }
  window.picker_scroll = picker_scroll;
  options.onReady && options.onReady($picker, {picker_scroll})
