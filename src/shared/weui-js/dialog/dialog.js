...
const tpl = `<div class="<%=className%>">
    ...
    <div class="weui-dialog <% if(isAndroid){ %> weui-skin_android <% } %>">
        ...
        <% if(useInput){ %>
            <div class="dialog-input"><input value="<%=inputVal%>" class="prompt-input" type="<%=inputType%>" /></div>
        <% } %>
        </div>
 ...
 * @param {string=} options.useInput 是否输入
  ...
  $dialogWrap
    .on('click', '.weui-dialog__btn', function (evt) {
      ...
      if (options.buttons[index].onClick) {
        const val = options.useInput ? $(".prompt-input").val() : "";
        if (options.buttons[index].onClick.call(this, evt, val, hide) === false) {
