    ...
    return new Promise((resolve, reject) => {
      ...
      qr.decodeFromImage(dataURL).then((res) => {
        if (res) {
          ...
          reject(Error("QrcodeDecoder res is false"))
        }
      }).catch(reject);
    })
  }
...
export default class QrScanner {
  ...
  constructor(
    ...
    this.$video = video;
    this._useBarcodeDetector = useBarcodeDetector
    ...
    this._qrEnginePromise = (this._useBarcodeDetector && 'BarcodeDetector' in window) ? QrScanner.createQrEngine() : QrScanner.createJsQrEngine();
  }
  ...
  static createQrEngine(worker) {
    return (('BarcodeDetector' in window) ? BarcodeDetector.getSupportedFormats() : Promise.resolve([]))
      .then((supportedFormats) => supportedFormats.indexOf('qr_code') !== -1
        ...
        : new worker({formats: ['qr_code']})
      );
  }

  /* async */
  static createJsQrEngine() {
    return Promise.resolve().then(() => new JsQrEngine());
  }
