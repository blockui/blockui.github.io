...
import {desDecrypt} from "shared/functions/crypto";
...
class ResultPage extends React.PureComponent {
  ...
  render() {
    const {result, desKey, encrypt} = this.state;
    return (
            ...
            </WeFlexItem>
            {
              ...
              <WeFlexItem>
                <WeButton primary={false} mini={true} width={"80%"} onClick={() => {
                  const result1 = desDecrypt(result, desKey)
                  if (!result1) {
                    return showTopTips("解密失败")
                  }
                  locationHashPost("Crypto/Des/Result", {encrypt: false}, {desKey, result:result1})
                }}>解密</WeButton>
              </WeFlexItem>
            }
