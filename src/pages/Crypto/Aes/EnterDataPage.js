...
import {aesDecrypt, aesEncrypt} from "shared/functions/crypto";

class EnterDataPage extends React.PureComponent {
  constructor(props) {
    super(props);
    const {aesKey,data} = props.location.postData || {}
    this.state = {
      aesKey,
      data,
  ...
  onEncrypt(encrypt) {
    ...
    const result = encrypt ? aesEncrypt(data, aesKey) : aesDecrypt(data, aesKey)
    if(!encrypt && !result){
      return showTopTips("解密失败")
    }
    locationHashPost("Crypto/Aes/Result", {encrypt}, {aesKey,result})
  }
